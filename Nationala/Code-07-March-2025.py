...
class PID_Controller(object):
    def __init__(self, noise: int, amp: int) -> None:

        self.kp: float = 0; self.ki: float = 0; self.kd: float = 0; self.dt: int = 0

        self.error: int = 0; self.previousError: int = 0
        self.proportional: float = 0; self.integral: float = 0
        self.derivative: float = 0; self.controllerOutput: int = 0

        self.sumerrors: int = 0

        self.boundlimit: int = 100000000 # Integral windup

        self.costf: float = 0 # Cost function based on the constant beta
        self.beta: float = 0.75 # Smoothing factor for cost function
        self.learning_rate: float = 0.00001 # Learning rate for gradient descent

        self.updatefreq: int = 25; self.iterator: int = 0

        # self.newkp: float = 0 # Tuned Kp value that are updated every self.updatefreq
        # self.newki: float = 0 # Tuned Ki value that are updated every self.updatefreq
        # self.newkd: float = 0 # Tuned Kd value that are updated every self.updatefreq

        self.noiseScale: int = noise # Remove noise from the input
        self.amplifier: int = amp # Scale up the input

        return None

    def reinitialize(self) -> None:
        self.error = 0; self.previousError = 0
        self.proportional = 0; self.integral = 0
        self.derivative = 0; self.controllerOutput = 0
        self.sumerrors = 0
        return None # Reinitialize the PID Controller

    def limit(self, value: int, valueMax: int, valueMin: int) -> int:
        return min(max(value, valueMin), valueMax)

    def setconstants(self, kp: float, ki: float, kd: float, dt: int, integralLimit: int) -> None:
        if(self.kp != kp or self.ki != ki or self.kd != kd or self.dt != dt or self.boundlimit != integralLimit):
            self.newkp, self.newki, self.newkd = kp, ki, kd
            self.kp, self.ki, self.kd = kp, ki, kd; self.dt = dt;
            self.boundlimit = integralLimit
        return None # Set PID Constants

    def gradient_descent_based_tuning(self) -> None:
        # Update the cost function
        self.costf = self.costf * self.beta + self.error * self.error * (1 - self.beta)

        self.iterator += 1
        if(self.iterator >= self.updatefreq):

            # Compute gradients using the power rule of the derivatives (E^2 dE = 2E)
            d_kp = -2 * self.error * self.proportional
            d_ki = -2 * self.error * self.integral
            d_kd = -2 * self.error * self.derivative

            # Update PID constants
            self.kp -= self.learning_rate * d_kp
            self.ki -= self.learning_rate * d_ki
            self.kd -= self.learning_rate * d_kd

            self.iterator = 1

            print(self.kp, self.ki, self.kd)

        return None # Tuning algorithm

    def compute(self, error: int, sign: int) -> int:
        self.error = (error // self.noiseScale) * self.amplifier
        self.sumerrors += self.error * self.dt
        self.sumerrors = self.limit(self.sumerrors, self.boundlimit, -self.boundlimit)

        self.proportional = self.kp * self.error
        self.integral = self.ki * self.sumerrors
        self.derivative = self.kd * (self.error - self.previousError)

        self.previousError = self.error; # Derivative update & Integral Windup

        self.controllerOutput = sign * int(
            self.proportional +
            self.integral +
            self.derivative
        )

        self.gradient_descent_based_tuning()
...
class DriveBase(object):
    ...
    def __init__(self, leftMotorDB: int, rightMotorDB: int, leftMotorSYS: int, rightMotorSYS: int, oneUnit: int, scaleUnit: int) -> None:
        ...
        self.kLeft: int = 0; self.kRight: int = 0; self.angle: int = 0
        self.controllerOutput: int = 0

        self.PID = PID_Controller(noise = 10, amp = 1)
    ...
    async def initRun(self) -> None:
        ...
        self.kLeft = 0; self.kRight = 0; self.angle = 0
    ...
    async def gyroForwards(self, distance: int, lowestSpeed: int, highestSpeed: int, *, kp: float = 0.6, ki: float = 0.002, kd: float = 0.4, dt: int = 5, integralLimit: int = 100000000, stallDetectionIterator: int = 500, stallDetectionThreshold: int = 5, accelerationScale: int = 100, easingMethod = SpeedMethods.easeOutInQuad, stop: bool = True, brakeMethod = motor.SMART_BRAKE) -> None:
        ...
        constantScale: All of our constants are written as fractions so that the PID Controller will have a faster response time. (deprecated for autotuning)
        ...
        self.PID.setconstants(kp, ki, kd, dt, integralLimit)
        self.PID.reinitialize()
        ...
        while(self.position < self.reachDistance):
            ...
            self.controllerOutput = self.PID.compute((self.reachAngle - self.angle), 1)
...
class Programs(object):
    ...
    async def Run1(self) -> None:
        ...
        # ---> Code <---
        await motor.run_for_degrees(driveBase.rightMotorSYS, -400, 700)
        await runloop.sleep_ms(500)
        await driveBase.gyroForwards(520, 500, 600, kp = 70, ki =  0, kd = 0.45)
        await runloop.sleep_ms(250)
        await motor.run_for_degrees(driveBase.leftMotorSYS, 1500, -1000)
        await runloop.sleep_ms(250)
        motor.run_for_degrees(driveBase.rightMotorSYS, 400, 700)
        await motor.run_for_degrees(driveBase.leftMotorSYS, 1500, 1000)
        await driveBase.gyroBackwards(500, 1000, 1000, kp = 0, ki = 0, kd = 0)
        '''await motor.run_for_degrees(driveBase.leftMotorSYS, 1000, -750)
        await motor.run_for_degrees(driveBase.rightMotorSYS, -600, -700)
        await driveBase.gyroBackwards(500, 500, 500, kp = 0.75, ki = 0.0005, kd = 0.4)'''
    ...
    async def Run2(self) -> None:
        ...
        # ---> Code <--- + ---> Go collect elements <---
        
        # ---: Go to :---
        await motor.run_for_degrees(driveBase.rightMotorSYS, -200, 500)
        await runloop.sleep_ms(250)
        await driveBase.turnRight(330, 325, 325, kLeft = 1, kRight = 0, error = 110)
        await runloop.sleep_ms(250)
        await driveBase.gyroForwards(695, 750, 1000, kp = 0.65, ki = 0.0004, kd = 0.451)
        await runloop.sleep_ms(250)
        await driveBase.turnLeft(250, 250, 250, kLeft = 0, kRight = 1)

        # ---: Align :---
        await runloop.sleep_ms(250)
        await motor.run_for_degrees(driveBase.rightMotorSYS, 800, 500)
        await driveBase.gyroForwards(100, 525, 525, kp = 0.65, ki = 0.0004, kd = 0.451)
        motor.run_for_degrees(driveBase.rightMotorSYS, -800, 500)
        await motor.run_for_degrees(driveBase.leftMotorSYS, 375, 750)
        await runloop.sleep_ms(250)

        # ---: Retrieve :---
        await motor.run_for_degrees(driveBase.leftMotorSYS, -400, 500)
        # await driveBase.gyroBackwards(50, 200, 200, kp = 0.65, ki = 0.0004, kd = 0.451)
        await driveBase.arcTurnBackwards(900, driveBase.rightMotorDB, 425, 1000, 1000, error = 2)
    ...
    async def Run3(self) -> None:
        ...
        # ---> Code <--- + ---> Align to boat <---
        await driveBase.gyroForwards(175, 500, 1000, kp = 0.6, ki = 0.00005, kd = 0.45)
        await runloop.sleep_ms(250)
        await driveBase.turnRight(280, 200, 200, kLeft = 1, kRight = 0, error = 85)
        await runloop.sleep_ms(250)
        await driveBase.gyroForwards(300, 250, 1000, kp = 0.6, ki = 0.00005, kd = 0.45)
        await runloop.sleep_ms(250)
        await motor.run_for_degrees(driveBase.leftMotorSYS, 300, 500)
        await driveBase.turnRight(605, 225, 225, kLeft = 1, kRight = 0, error = 87)
        await runloop.sleep_ms(250)

        # ---> Retrieve treasure and complete boat <---
        await driveBase.gyroForwards(317, 525, 600, kp = 0.6, ki = 0.00005, kd = 0.45)
        await motor.run_for_degrees(driveBase.leftMotorSYS, -450, 500)
        await runloop.sleep_ms(250)
        await driveBase.gyroBackwards(300, 350, 400, kp = 0.6, ki = 0.00005, kd = 0.45, easingMethod = SpeedMethods.linear)
        await motor.run_for_degrees(driveBase.leftMotorSYS, -580, 1000)
        await driveBase.turnRight(1125, 325, 325, kLeft = 1, kRight = -1, error = 100)
        await driveBase.gyroForwards(375, 1000, 1000, kp = 0.6, ki = 0.00005, kd = 0.45)
        # motor.run_for_degrees(driveBase.leftMotorSYS, 200, 1000)
        await runloop.sleep_ms(250)
    ...
    async def Run4(self) -> None:
        ...
        # ---> Code <---
        await driveBase.gyroForwards(572, 725, 1000, kp = 0.45, ki = 0.00025, kd = 0.45)
        
        await runloop.sleep_ms(250)
        await driveBase.turnLeft(500, 237, 237, kLeft = 0, kRight = 1, error = 75)
        
        await runloop.sleep_ms(250)
        await driveBase.gyroForwards(175, 750, 750, kp = 0.45, ki = 0.00025, kd = 0.45)
        await motor.run_for_degrees(driveBase.leftMotorSYS, -1700, 1000)
        
        await runloop.sleep_ms(250)
        await driveBase.gyroBackwards(10, 750, 750, kp = 0.45, ki = 0.00025, kd = 0.45)
        await motor.run_for_degrees(driveBase.leftMotorSYS, 1600, 700)
        await driveBase.gyroBackwards(150, 750, 750, kp = 0.45, ki = 0.00025, kd = 0.45)

        await driveBase.turnRight(300, 325, 325, kLeft = 1, kRight = -1, error = 100)
        await runloop.sleep_ms(250)
        await driveBase.gyroBackwards(800, 1000, 1000, kp = 0, ki = 0, kd = 0)
    ...
    async def Run5(self) -> None:
        await driveBase.initRun()
        await driveBase.gyroBackwards(600, 700, 800, kp = 0.48, ki = 0, kd = 0.45)
        await driveBase.gyroBackwards(745, 300, 450, kp = 45, ki = 0, kd = 0.45)#45
        await runloop.sleep_ms(250)
        
        await driveBase.gyroBackwards(85, 800, 900, kp = 0.45, ki = 0, kd = 0.45)
        await runloop.sleep_ms(500)
        await driveBase.turnRight(800, 1000, 1000, kLeft = 1, kRight = 0, error = 75)

        await driveBase.gyroForwards(400, 725, 1000, kp = 0.48, ki = 0, kd = 0.45)
        #300,400 viteza
        # ---> Code <---
    ...
    async def Run6(self) -> None:
        await driveBase.initRun()
        await driveBase.turnLeft(50, 250, 250, kLeft = 0, kRight = 1)
        await runloop.sleep_ms(250)
        await driveBase.gyroForwards(422, 700, 800, kp = 4.031552, ki = 0, kd = 0.449982)
        await runloop.sleep_ms(625)

        await driveBase.gyroForwards(280, 500, 600, kp = 0.46, ki = 0, kd = 0.45)
        await runloop.sleep_ms(250)
        await driveBase.turnRight(1060, 225, 225, kLeft = 1, kRight = -1, error = 75)
        await driveBase.gyroForwards(80, 500, 500, kp = 0.46, ki = 0, kd = 0.45)
        await driveBase.gyroBackwards(50, 400, 400, kp = 0, ki = 0, kd = 0)
        await runloop.sleep_ms(250)

        await driveBase.turnRight(800, 225, 225, kLeft = 1, kRight = -1, error = 75)
        await driveBase.gyroForwards(800, 1000, 1000, kp = 0.46, ki = 0, kd = 0.45)
        # ---> Code <---
    ...
    async def Run7(self) -> None:
        ...
        await driveBase.initRun()
        await driveBase.gyroForwards(745, 500, 800, kp = 0.4516163, ki = 0.0004192375, kd = 0.45)
        await runloop.sleep_ms(250)
        await driveBase.turnRight(390, 125, 125, kLeft = 1, kRight = -1, error = 75)
        await runloop.sleep_ms(250)
        await driveBase.gyroForwards(115, 190, 190, kp = 0.4516163, ki = 0.0004192375, kd = 0.45)
        await runloop.sleep_ms(500)

        await driveBase.gyroBackwards(240, 450, 450, kp = 0.4516163, ki = 0.0004192375, kd = 0.45)
        await runloop.sleep_ms(250)
        await driveBase.turnLeft(350, 250, 250, kLeft = 0, kRight = 1)
        await runloop.sleep_ms(250)
        await motor.run_for_degrees(driveBase.leftMotorSYS, 230, 750)
        await runloop.sleep_ms(250)
        await driveBase.gyroForwards(160, 300, 600, kp = 0.4516163, ki = 0.0004192375, kd = 0.45)
        await motor.run_for_degrees(driveBase.leftMotorSYS, -130, 750)
        await driveBase.gyroBackwards(150, 700, 700, kp = 0.4516163, ki = 0.0004192375, kd = 0.45)
        
        await driveBase.turnLeft(350, 250, 250, kLeft = 0, kRight = 1)
        await driveBase.gyroBackwards(600, 1000, 1000, kp = 0.4516163, ki = 0.0004192375, kd = 0.45)
    ...
    async def Run8(self) -> None:
        await driveBase.initRun()
        await driveBase.gyroForwards(620, 700, 800, kp = 0.453154, ki = 0.0008540225, kd = 0.45)
        await driveBase.gyroBackwards(500, 1000, 1000, kp = 0.4516163, ki = 0.0008540225, kd = 0.45)
        #await driveBase.gyroForwards(200, 1000, 1000, kp = 0.4556837, ki = 0.01805082, kd = 0.45)
    ...
    async def Run9(self) -> None:
        ...
        await driveBase.turnLeft(480, 250, 250, kLeft = 0, kRight = 1, error = 75)
        await runloop.sleep_ms(250)
        await driveBase.gyroForwards(1050, 700, 1000, kp = 0.46, ki = 0.002, kd = 0.45)
        await motor.run_for_degrees(driveBase.leftMotorSYS, 300, 750)
        await runloop.sleep_ms(250)
        await motor.run_for_degrees(driveBase.leftMotorSYS, -300, 750)
        await driveBase.arcTurnBackwards(360, driveBase.rightMotorDB, 410, 1000, 1000, error = 2)
        # ---> Code <---
